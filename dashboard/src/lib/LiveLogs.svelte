<script lang="ts">
  ...
  $: filteredLogs = $logs.filter(log => {
    ...
    const matchesAction = selectedAction === 'All' ||
      ...
      (selectedAction === 'RATE_LIMIT' && (log.action === 'RATE_LIMIT' || log.action === 'LIMIT' || log.action === 'RATE')) ||
      (selectedAction === 'PASS' && (log.action === 'PASS' || log.action === 'ALLOW' || log.action === 'REDIRECT'));
...
<div class="live-logs-panel flex flex-col h-full gap-lg">
  ...
  <!-- TOOLBAR / FILTERS -->
  <div class="glass-panel p-md rounded-xl flex flex-col md:flex-row justify-between items-center gap-md">
    <div class="flex flex-1 w-full md:w-auto items-center gap-md">
      <div class="relative flex-1 max-w-md">
        <span class="material-symbols-outlined absolute left-md top-1/2 -translate-y-1/2 text-on-surface-variant text-sm">search</span>
        <input 
    ...
    <div class="flex items-center gap-md w-full md:w-auto justify-end">
      ...
      <div class="flex flex-col items-end">
        <span class="text-[10px] text-on-surface-variant uppercase tracking-tighter">Filtered Events</span>
        <span class="text-primary font-metric-lg text-headline-md leading-none">{filteredLogs.length}</span>
      </div>
    </div>
  </div>
  ...
  <div class="glass-panel flex-1 rounded-xl overflow-hidden flex flex-col">
    <!-- Table Header -->
    <div class="bg-surface-container/50 border-b border-outline-variant px-lg py-sm grid grid-cols-12 gap-md items-center text-[11px] uppercase font-bold tracking-wider">
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar font-code-md text-code-md">
      {#each paginatedLogs as log}
        <div class="terminal-row {getRowClass(log.action)} px-lg py-md grid grid-cols-12 gap-md items-center transition-colors">
          ...
          <div class="col-span-1"><span class="px-sm py-0.5 rounded bg-surface-container text-[10px] font-bold font-sans">{log.method}</span></div>
          ...
          <div class="col-span-3 text-right text-on-surface-variant truncate font-sans" title={log.reason}>{log.reason}</div>
        </div>
      {:else}
    ...
    <!-- TABLE FOOTER -->
    <div class="bg-surface-container-low border-t border-outline-variant px-lg py-xs flex justify-between items-center text-xs">
      ...
      {#if totalPages > 1}
        <div class="flex gap-sm items-center">
          ...
          <span class="text-on-surface px-md text-code-md font-bold">{currentPage} / {totalPages}</span>
          <button 
  ...
  <!-- FOOTER / STATS -->
  <footer class="px-lg py-md bg-surface-container-lowest border-t border-outline-variant flex gap-xl overflow-x-auto custom-scrollbar rounded-xl">
    <div class="flex-shrink-0 flex items-center gap-md">
      ...
      <div>
        <div class="text-[10px] text-on-surface-variant uppercase font-bold tracking-widest">Blocks (24h)</div>
        <div class="text-headline-md font-metric-lg text-on-surface">{formatCount($stats.blocked)}</div>
      </div>
    </div>
    ...
    <div class="flex-shrink-0 flex items-center gap-md">
      ...
      <div>
        <div class="text-[10px] text-on-surface-variant uppercase font-bold tracking-widest">Rate Limits</div>
        <div class="text-headline-md font-metric-lg text-on-surface">{formatCount($stats.rate_limited)}</div>
      </div>
    </div>
    ...
    <div class="flex-shrink-0 flex items-center gap-md">
      ...
      <div>
        <div class="text-[10px] text-on-surface-variant uppercase font-bold tracking-widest">Total Inspected</div>
        <div class="text-headline-md font-metric-lg text-on-surface">{formatCount($stats.total_requests)}</div>
      </div>
    </div>
  </footer>
</div>
